...
public type Spreadsheet client object {
    ...
    //  #
    //  # + name - New name for the copy of the Spreadsheet
    //  # + return - Spreadsheet client object on success, else returns an error
    //  public remote function duplicate(string? name = "") returns Spreadsheet|error {
    //1) Use the Drive API's files.copy method. The fileId in the Drive API is equivalent to the spreadsheetId in the Sheets API.
          //
          //2) Don't use a "template" spreadsheet that you copy. Instead, use the Sheet API's spreadsheets.create method.
         // You can use spreadsheets;.get to retrieve your "template" JSON, and can tweak that as necessary before
         //creating your new spreadsheet.
    //  }
     ...
     # Renames the Spreadsheet with the given name.
      ...
      public remote function rename(string name) returns error? {
            http:Request request = new;
          json sheetJSONPayload = {"requests": [{"updateSpreadsheetProperties": {"properties": {"title": name},
          "fields": "title"}}]};
          request.setJsonPayload(sheetJSONPayload);
          string deleteSheetPath = SPREADSHEET_PATH + PATH_SEPARATOR + self.spreadsheetId + BATCH_UPDATE_REQUEST;
          var httpResponse = self.httpClient->post(deleteSheetPath, request);
